...
module.exports = (grunt) ->
  ...
  grunt.initConfig
    ...
    copy:
      ...
      appMacosAssets:
        files: [
          expand: true
          cwd: "<%= config.app %>"
          dest: "<%= config.dist %>/BitchTv.app/Contents/Resources/app.nw"
          src: "**"
        ]
    ...
    compress:
      ...
      finalWindowsApp:
        options:
          archive: "<%= config.dist %>/bitch-tv-prog-window-v0.0.1.zip"
      ...
      finalMacApp:
        options:
          archive: "<%= config.dist %>/bitch-tv-prog-macos-v0.0.1.zip"
  ...
  grunt.registerTask "chmod", "Add lost Permissions.", ->
    fs = require("fs")
    fs.chmodSync "dist/BitchTv.app/Contents/Frameworks/node-webkit Helper EH.app/Contents/MacOS/node-webkit Helper EH", "555"
    fs.chmodSync "dist/BitchTv.app/Contents/Frameworks/node-webkit Helper NP.app/Contents/MacOS/node-webkit Helper NP", "555"
    fs.chmodSync "dist/BitchTv.app/Contents/Frameworks/node-webkit Helper.app/Contents/MacOS/node-webkit Helper", "555"
    fs.chmodSync "dist/BitchTv.app/Contents/MacOS/node-webkit", "555"
    return
  ...
  grunt.registerTask "launchMacApp", "Launch mac application", ->
    ...
    childProcess = require("child_process")
    exec = childProcess.exec
    exec "open ./dist/BitchTv.app", (error, stdout, stderr) ->
      result = true
      grunt.log.write stdout  if stdout
      grunt.log.write stderr  if stderr
      if error isnt null
